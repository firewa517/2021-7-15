<template>
  <div>
    ...
    <!-- 对话框 -->
    <el-dialog title="添加菜单" center :visible.sync="dialogFormVisible" :before-close="beforeClose">
      <el-form
        :model="ruleForm"
        :rules="rules"
        ref="ruleForm"
      ...
      >
        ...
        <el-form-item label="上级菜单" prop="pid">
          <el-select v-model="ruleForm.pid" placeholder="请选择上级菜单">
            ...
            <el-option v-for="option in tableData" :label="option.title" :key="option.id" :value="option.id"></el-option>
          </el-select>
        </el-form-item>
...
export default {
  ...
  methods: {
    //封装一个查询菜单列表接口
    //不传递任何参数的情况 直接获取普通数据结构，如果加上istree这个参数，那么我们获取的是树形数据结构
    getDataList() {
      this.$axios({
        url: "/api/menulist",
        params:{
